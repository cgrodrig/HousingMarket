...
3. fit a linear model with all of the predictors that are linear plus yearly, monthly, and daily affect (to do this we would need a year, month, and day column)

# rental prices
```{r}
#rental_price <-
  #read_csv("leiguskra.csv", locale = locale(encoding = "ISO-8859-1"))
...
rental_price_c <- leiguskra %>%
  mutate(Date = as_date(THINGLYSTDAGS)) %>%
  group_by(Date, SVEITARFELAG) %>%
  mutate(Inventory = n()) %>%
  dplyr::select(Date, SVEITARFELAG, Inventory) %>%
...
#full_data <- complete_data_out6 %>% 
 # left_join(rental_price_c, 
 #            by = c("Date" = "Date", "sveitarfelag" = "SVEITARFELAG")) %>% 
 # left_join(interest_rate, by = c("Date" = "date")) %>% 
 # distinct()
```
