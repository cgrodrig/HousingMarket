...
housesales_c <- housesales %>% 
  ...
  dplyr::select(-c(faerslunumer, emnr, skjalanumer, fastnum, heimilisfang, heinum, svfn, sveitarfelag, thinglystdags, kaupverd, fasteignamat, fepilog, lod_flm, lod_flmein, fullbuid, onothaefur_samningur)) %>%
  filter(!is.na(byggar))

colnames(housesales_c) <- c('postalcode','date', 'age', 'msq', 'type', 'outcome')

housesales_s <- subset(housesales_c, type %in% c("Fjölbýli      ", "Einbýli       ", "Sérbýli       "))

housesales_f <- filter(housesales_s, msq > 20 & msq < 200 )
